#include "frameadsb.h"
#include "ui_frameadsb.h"

#include <QDebug>
#include <QDateTime>

FrameADSB::FrameADSB(QWidget *parent) :
    QFrame(parent),
    ui(new Ui::FrameADSB)
{
    ui->setupUi(this);
    ...
    ui->tableViewADSBInf->setModel(model);

    timer->start(1000);

}
void FrameADSB::timerTimeout()
{
    QHashIterator<QString,quint64> i(target_time_tag_list);
    QStringList target_to_delete;
    ...
    for(int i=0;i<target_to_delete.size();i++)
    {
        ...
        QList<QStandardItem *> listTarget = model->findItems(target_to_delete.at(i),0);
    ...
    if(m_mqtt->isConnected())
    {
        ...
        {
            QString id,rng,brn,spd,crs,alt,mq_data;
            ...
            index = model->index(dataCount_mqtt,5);
            alt = model->data(index).toString();

            mq_data = id+"#"+rng+"#"+brn+"#"+spd+"#"+crs+"#"+alt;
            mq_databyte = mq_data.toUtf8();
            m_mqtt->publish(m_mqtt->getMID(), "adsb", mq_databyte.size(),(const unsigned char*) mq_databyte.data(), 2, false);
...
void FrameADSB::trigger_target_update(QString icao, double spd, double crs, double rng, double brn,double alt)
{
    if(rng<50)
    {

    quint64 new_target_tt;
//    qDebug()<<Q_FUNC_INFO<<icao<<spd<<crs<<rng<<brn;
    if(target_time_tag_list.contains(icao))
    {
        ...
        QList<QStandardItem *> listTarget = model->findItems(icao,0);
        if(!listTarget.isEmpty())
        {
...
//                           QString::number(qrand() % (20 - 3) + 3));
            ...
            insertList(icao, spd, crs, rng, brn,alt);
    }
    else
    {
        target_time_tag_list.insert(icao,QDateTime::currentMSecsSinceEpoch());
        insertList(icao, spd, crs, rng, brn,alt);
    }
    }
}
void FrameADSB::insertList(QString icao, double spd, double crs, double rng, double brn,double alt)
{
    ...
    model->setData(model->index(model->rowCount()-1,4,QModelIndex()),
                   QString::number(crs,'f',1));
    model->setData(model->index(model->rowCount()-1,5,QModelIndex()),
                   QString::number(alt,'f',1));
    ...
    model->item(model->rowCount()-1,5)->setTextAlignment(Qt::AlignHCenter|Qt::AlignVCenter);
    }
}
FrameADSB::~FrameADSB()
{
